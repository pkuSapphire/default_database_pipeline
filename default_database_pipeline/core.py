...
def get_wrds_conn(username, password):
    global WRDS_CONN
...
def get_gvkey(username, password, filename=os.path.join(DATA_DIR, "gvkey_data.pkl")):
    """
    ...
    if os.path.exists(filename):
        ...
        conn = get_wrds_conn(username, password)
...
def get_ratings(username, password, filename=os.path.join(DATA_DIR, "ratings_data.pkl")):
    """
...
def get_sector(ratings_merged, username, password, filename=os.path.join(DATA_DIR, "sector_data.pkl")):
    """
...
def get_sector_info(ratings_merged, username, password):
    """

    Merge sector data (SIC-based) and GICS info, then create an overall 'sector' column.
    """
    info = get_sector(ratings_merged, username, password)
...
def get_financials(username, password, filename=os.path.join(DATA_DIR, "financials_data.pkl")):
    """
...
def prepare_financials(username, password):
    """

    Sort the financials by date descending for each gvkey.
    """
    fin = get_financials(username, password)
...
def merge_financials_ratings(financials_df, ratings_df):
    ...
    # Add blank row for the year after the last available fyear per gvkey
    latest_rows = fin_sub.sort_values("datadate").groupby("gvkey").tail(1)
    added_rows = latest_rows.copy()
    added_rows["fyear"] = added_rows["fyear"] + 1
    added_rows["datadate"] = pd.to_datetime(added_rows["datadate"], errors="coerce")
    added_rows["datadate"] = added_rows["datadate"] + pd.DateOffset(years=1)
    financial_cols = added_rows.columns.difference(["gvkey", "fyear", "datadate"])
    added_rows[financial_cols] = np.nan
    fin_sub = pd.concat([fin_sub, added_rows], ignore_index=True)
    ...
    subset = merged_fin_df[merged_fin_df["gvkey"] == "006127"]
    # Ensure all columns and rows are shown in the console
    _pd.set_option('display.max_columns', None)
    _pd.set_option('display.width', None)
    print("==== gvkey=006127 financials preview ====")
    #
    # print(merged_fin_df[merged_fin_df["gvkey"] == "006127"])
...
def compute_default_dates(merged_fin_df):
    ...
    # print(default_df[default_df["gvkey"]=="006127"])
    return default_df
...
def run_pipeline(username, password):
    """
    ...
    # calculate_auc(df)
    # save final dataset
    df.to_pickle(os.path.join(DATA_DIR, "cleaned_dataset.pkl"))
    print(sum(df["dflt_flag"]))

def start_pipeline(custom_data_dir=None):
    """
